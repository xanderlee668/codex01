...
- **行程与群聊**
  ...
  - `MarketplaceViewModel.createTrip` 使用 `POST /api/trips` 将新行程提交到服务器，成功后立即插入本地列表并触发一次刷新以确保和数据库保持一致。请求体会同时携带 `organizer_id` 与 `organizerId` 字段，便于后端既可以依赖 JWT，也可以直接读取组织者 ID；还会并行写入 `start_date`/`start_time`、`min_participants`/`minimum_participants`、`cost_per_person`/`estimated_cost_per_person` 等字段，以兼容后端不同的命名风格。若后端仅返回状态码或成功消息，前端会自动回退到 `GET /api/trips` 重新同步最新行程。【F:Sources/ViewModels/MarketplaceViewModel.swift†L334-L414】【F:Sources/Networking/APIClient.swift†L168-L283】
